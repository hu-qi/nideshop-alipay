...
class _C extends Taro.Component {
  ...
  getAddressDetail = () => {
    let that = this
    util
      .request(api.AddressDetail, { id: that.data.addressId })
      .then(function(res) {
        if (res.errno === 0) {
          that.setData({
            address: res.data
  ...
  selectRegion = event => {
    ...
    that.setData({
      regionList: that.data.regionList.map(item => {
        //标记已选择的
        if (
          that.data.regionType == item.type &&
          that.data.selectRegionList[that.data.regionType - 1].id == item.id
        ) {
          item.selected = true
        } else {
          item.selected = false
  ...
  render() {
    const {
      ...
      selectRegionList: selectRegionList,
      selectRegionDone: selectRegionDone,
    ...
    return (
      <Block>
        <View className="add-address">
          <View className="add-form">
            ...
            <View className="form-default">
              <Text
                onClick={this.bindIsDefault}
                className={
                  'default-input ' + (address.is_default == 1 ? 'selected' : '')
                }
              >
                设为默认地址
              </Text>
          ...
          <View className="btns">
            <Button className="cannel" onClick={this.cancelAddress}>
              取消
            </Button>
            <Button className="save" onClick={this.saveAddress}>
              保存
            </Button>
          </View>
          {openSelectRegion && (
        ...
        {openSelectRegion && (
          <View className="bg-mask" onClick={this.cancelSelectRegion} />
        )}
      </Block>
    )
  }
